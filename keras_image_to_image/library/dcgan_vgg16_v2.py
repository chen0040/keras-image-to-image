...
class DCGanWithVGG16V2(object):
    ...
    def get_config_file_path(model_dir_path):
        return os.path.join(model_dir_path, DCGanWithVGG16V2.model_name + '-config.npy')

    @staticmethod
    def get_weight_file_path(model_dir_path, model_type):
        return os.path.join(model_dir_path, DCGanWithVGG16V2.model_name + '-' + model_type + '-weights.h5')
    ...
    def create_model(self):
        ...
        image_layer1 = Dense(1024)(img_input1)

        generator_layer = Activation('tanh')(image_layer1)
        ...
        self.generator = Model(img_input1, generator_output)
        ...
        self.model = Model(img_input1, model_output)
    ...
    def load_model(self, model_dir_path):
        config_file_path = DCGanWithVGG16V2.get_config_file_path(model_dir_path)
        self.config = np.load(config_file_path).item()
        self.img_width = self.config['img_width']
        self.img_height = self.config['img_height']
        self.img_channels = self.config['img_channels']
        self.create_model()
        self.generator.load_weights(DCGanWithVGG16V2.get_weight_file_path(model_dir_path, 'generator'))
        self.discriminator.load_weights(DCGanWithVGG16V2.get_weight_file_path(model_dir_path, 'discriminator'))
    ...
    def fit(self, model_dir_path, image_path_pairs, epochs=None, batch_size=None, snapshot_dir_path=None,
        ...
        for epoch in range(epochs):
            ...
            for batch_index in range(batch_count):
                ...
                generated_images = self.generator.predict(src_image_feature_batch, verbose=0)
                ...
                self.discriminator.trainable = False
                g_loss = self.model.train_on_batch(src_image_feature_batch, np.array([1] * batch_size))

                print("Epoch %d batch %d g_loss : %f" % (epoch, batch_index, g_loss))
                if (epoch * batch_size + batch_index) % 10 == 9:
                    self.generator.save_weights(DCGanWithVGG16V2.get_weight_file_path(model_dir_path, 'generator'), True)
                    self.discriminator.save_weights(DCGanWithVGG16V2.get_weight_file_path(model_dir_path, 'discriminator'), True)
    ...
    def save_snapshots(self, generated_images, snapshot_dir_path, epoch, batch_index):
        ...
        img_from_normalized_img(image).save(
            os.path.join(snapshot_dir_path, DCGanWithVGG16V2.model_name + '-' + str(epoch) + "-" + str(batch_index) + ".png"))
